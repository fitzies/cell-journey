...
export function EventSection({ title, rows, showEmpty = false, limited = false, detailTab = 'attendance', showHeading = true }: { title: string; rows: FeedRow[]; showEmpty?: boolean; limited?: boolean; detailTab?: 'attendance' | 'schedule'; showHeading?: boolean }) {
  if (!rows.length && !showEmpty) return null;
  return (
    <View style={styles.feedSection}>
      {showHeading ? <View style={styles.feedHeading}>
        <SectionLabel>{title}</SectionLabel>
        <SectionCount count={rows.length} limited={limited} />
      </View> : null}
      <View style={styles.feed}>
        {rows.map((row) => (
          <EventCard key={row.event._id} row={row} detailTab={detailTab} />
        ))}
      </View>
    </View>
  );
...
function EventCard({ row, detailTab = 'attendance' }: { row: FeedRow; detailTab?: 'attendance' | 'schedule' }) {
  ...
  return <AttendanceEventCard detailTab={detailTab} event={row.event} kind={row.kind} status={status}
    feedSummary={{
